...
export class Income {
    ...
    async showCategories() {
        try {
            const categories = await FinancesService.getAnyCategories('income');
    ...
    async deleteCategory(categoryId) {
        try {
            const response = await FinancesService.deleteAnyCategory( 'income', categoryId);
            if (response) {
    ...
    openEditCategory(categoryId) {
        if (categoryId) {
            this.openNewRoute(`/income/edit?id=${categoryId}`);
        }
    }
}
